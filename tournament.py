...
def play_matches(cpu_agents, test_agents, num_matches):
    ...
    for idx, agent in enumerate(cpu_agents):
        ...
        if counts[1]>0:
             print("{} forfeits by {} ".format(counts[1],agent.player))
...
def main():
    ...
    test_agents2 = [
        Agent(AlphaBetaPlayer(score_fn=open_move_score,timeout=timeout["AB_Open"]), "AB_Open"),
        Agent(AlphaBetaPlayer(score_fn=center_score,timeout=timeout["AB_Center"]), "AB_Center"),
        Agent(AlphaBetaPlayer(score_fn=improved_score,timeout=timeout["AB_Improved"]), "AB_Improved")
      #  Agent(AlphaBetaPlayer(score_fn=custom_score_3), "AB_Custom_3")
    ]
